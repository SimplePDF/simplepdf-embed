...
const StateBadge = ({ state }: { state: ToolInvocationState }) => {
  ...
  if (state === 'output-available') {
    return (
      <span role="img" aria-label={t('toolInvocation.states.output-available')} className="text-emerald-500">
        <SuccessIcon />
      </span>
    )
  }
  if (state === 'output-error') {
    return (
      <span role="img" aria-label={t('toolInvocation.states.output-error')} className="text-rose-500">
        <ErrorIcon />
      </span>
    )
  }
  return (
    <span role="img" aria-label={t(`toolInvocation.states.${state}`)} className="text-slate-500">
    ...
    <div className="my-2 rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-xs">
      <div className="flex items-center justify-between">
        <span className="flex items-center gap-2.5 font-medium text-slate-800">
          ...
          {t(`toolInvocation.names.${toolName}`, {
