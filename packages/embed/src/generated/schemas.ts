...
export const DeleteFieldsInput = z.object({
  field_ids: z.array(z.string()).describe("IDs of the fields to delete. Omit to delete every field on the target page.").optional(),
...
export const FocusFieldInput = z.object({
  field_id: z.string().describe("ID of the field to focus and scroll into view."),
...
export const GetDocumentContentInput = z.object({
  extraction_mode: z.enum(["auto", "ocr"]).describe("Extraction strategy: 'auto' (default) or 'ocr' to force optical recognition.").optional(),
...
export const LoadDocumentInput = z.object({
  data_url: z.string().describe("The document to load, as a data URL."),
...
export const MovePageInput = z.object({
  from_page: z.number().int().describe("1-based current position of the page to move."),
  to_page: z.number().int().describe("1-based destination position for the page."),
