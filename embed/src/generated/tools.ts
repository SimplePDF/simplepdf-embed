...
export const TOOL_DEFINITIONS = {
  createField: { description: "Create a new overlay field of the given type at an (x, y) position and size, in PDF points with a top-left origin (y grows downward; the same convention get_fields reports, so created geometry reads back identically). Placed on a 1-based page. COMB_TEXT fields accept an explicit comb cell layout. To adjust an existing field, delete it and recreate it with the corrected geometry. Returns { field_id } for the created field. Requires editing to be enabled.", inputSchema: Schemas.CreateFieldInput },
  ...
  getFields: { description: "List every fillable field in the loaded document, including native dropdown and radio AcroFields. Each field reports its id, name, type, page, current value, and geometry: x, y, width, height in PDF points with a top-left origin (y grows downward) - the same convention create_field consumes, so a create-then-read round-trip returns identical numbers. COMB_TEXT fields also report their comb cell layout. Call this first to discover field ids before reading or setting values. Pass include_annotated_pages to additionally get badge-numbered page renders for visual verification. Returns { fields, pages, annotated_pages? } where pages lists every page with its current 1-based position and its displayed size in PDF points.", inputSchema: Schemas.GetFieldsInput },
